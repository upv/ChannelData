...
This script BATCH-GENERATES the channel for every combination of
  * channel model : UMa, UMi, CDL-B, CDL-C
  * IoT [dB]      : 0, 10, 20
  * received SNR  : -20, 0, 20 dB
and for each combination saves a 4-panel picture
  (localization, channel magnitude, covariance matrix, eigenmodes).
The per-model channel tensors are stored in .npz files with matching names.
...
# ------------------- 4-panel picture for one operating point -------------- #
def make_picture(model_name, H_mag_db, R_sig, R_in, R_rx, iot_db, snr_db):
    ...
    iot_achieved = 10 * np.log10(np.trace(R_in).real / NUM_BS_ANT)

    fig, axes = plt.subplots(2, 2, figsize=(15, 13))
    axm, axg, axc, axe = axes[0, 0], axes[0, 1], axes[1, 0], axes[1, 1]
    fig.suptitle(f"{model_name}   |   IoT = {iot_db:.0f} dB,  SNR = {snr_db:.0f} dB",
    ...
    axm.set_title("Localization: serving BS, served UE, inter-cell interferers")
    ...
    axc.set_title("BS received covariance "
                  ...
                  f"SNR = {snr_db:.0f} dB,  IoT = {iot_achieved:.1f} dB,  N=1")
    ...
    out_png = f"su_intercell_{model_name}_IoT{int(iot_db)}_SNR{int(snr_db)}.png"
...
for model_name in CHANNELS:
    ...
    for iot_db in IOT_LIST:
        ...
        for snr_db in SNR_LIST:
            ...
            R_rx  = R_sig + R_in
            png = make_picture(model_name, H_mag_db, R_sig, R_in, R_rx,
                               iot_db, snr_db)
            print(f"    IoT={iot_db:5.1f} dB  SNR={snr_db:6.1f} dB  ->  {png}")

n_pics = len(CHANNELS) * len(IOT_LIST) * len(SNR_LIST)
print(f"\nDone: {n_pics} pictures and {len(CHANNELS)} channel files saved.")
